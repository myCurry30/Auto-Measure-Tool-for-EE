...
`web\client\.env` 已预置为本地模式，访问地址固定为 `http://localhost:8200`。无需修改。
...
- 检查服务是否启动：`Get-ScheduledTask -TaskName "EE AutoTool Web"`
- 检查 `npm run build` 是否执行成功（`web/client/dist/` 目录是否存在）
- 确认访问地址是 `http://localhost:8200`（不是服务器 IP）

### Q2: 登录提示"当前设备未注册"

- 本机的 MAC 地址不在 `user_pins.json` 中
- 在本机 `ipconfig /all` 获取 MAC 地址，更新 `user_pins.json`
- 配置文件修改后无需重启，下次登录自动生效

### Q3: 登录提示"正在获取设备信息，请重试"但一直重试

- 本地 `ipconfig` 确认网卡启用了 IPv4
- 重启服务后再试

### Q4: 连接示波器失败

- 确认本机已安装 NI-VISA
- 确认示波器已开机且线缆连接正常（线缆插在本机）
- 如果是 IP 连接：确认本机能 ping 通示波器 IP
...
| 管理员姓名 | ________ |
| 工程师姓名 | ________ |
| 项目路径 | `C:\EE_POWER_ON_Tool_WEB\autotool-web` |
| JWT 密钥 | ________ |

> 每位工程师在自己的 PC 上部署一套。部署完成后填写用户信息。
