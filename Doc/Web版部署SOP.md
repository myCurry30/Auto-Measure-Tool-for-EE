...
找到正在使用的网卡的 **IPv4 地址**（如 `10.31.133.57`）。确保此 IP 为固定 IP。
...
如果服务器 IP 或端口有变动，编辑此文件修改地址后重新 `npm run build`。
...
- 检查服务器 IP 是否可达：在工程师 PC 上 `ping <服务器IP>`
- 检查防火墙是否放行 8200 端口
...
- 在服务器上手动 ping 一次客户端 IP
- 然后再试登录

### Q4: 连接示波器失败

- 确认服务器上已安装 NI-VISA
- 确认示波器已开机且线缆连接正常（线缆插在服务器上）
- 如果是 IP 连接：确认服务器能 ping 通示波器 IP
