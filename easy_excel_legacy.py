"""Excel 操作类 - tkinter 版本.

提供 win32com Excel 自动化的简化接口。
"""
...
class EasyExcel:
    ...
    def close(self):
        try:
            ...
            print("[EasyExcel Legacy] Excel closed successfully")
        except Exception as e:
            print(f"[EasyExcel Legacy] Error closing Excel: {e}")
            # 确保资源被释放
            if hasattr(self, 'xlApp'):
                try:
                    self.xlApp.Quit()
    ...
    def activate_sheet(self, sheet_name):
        ...
        try:
            ...
            print(f"[EasyExcel Legacy] Activated sheet: {sheet_name}")
            return True
        except Exception as e:
            print(f"[EasyExcel Legacy] Error activating sheet {sheet_name}: {e}")
            return False
    ...
    def _activate_excel_window(self):
        ...
        try:
            ...
            try:
                ...
                if hwnd:
                    ...
                    if ctypes.windll.user32.ShowWindow(hwnd, 3):
                        print("[EasyExcel Legacy] Excel window maximized")
                    else:
                        # 如果最大化失败，设置一个较大的窗口
                        # 获取屏幕尺寸
                        ...
                        print(f"[EasyExcel Legacy] Excel window resized to {window_width}x{window_height}")

                    # 将窗口置于前台
                    ctypes.windll.user32.SetForegroundWindow(hwnd)
            except Exception as e:
                print(f"[EasyExcel Legacy] Error setting window size: {e}")
                # 如果设置大小失败，至少确保窗口可见
                pass
            ...
            print("[EasyExcel Legacy] Excel window brought to front")
        except Exception as e:
            print(f"[EasyExcel Legacy] Error bringing Excel to front: {e}")